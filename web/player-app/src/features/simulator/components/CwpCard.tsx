...
export function CwpCard({ cwp, mode, expanded, onToggle }: { cwp: CwpConfig; mode: SimulatorMode; expanded: boolean; onToggle: () => void }) {
  ...
  return <article className={'cwp-card' + (expanded ? ' expanded' : '')}>
    ...
    <div className="cwp-meta"><span>IP: {config.consoleIp}</span><span>Rádios: {config.radios}</span><span>Telefones: {config.telephones}</span></div>
    <div className="cwp-services">{config.services.map(service => <ServicePill key={service.id} service={service} />)}</div>
    {expanded && <div className="cwp-config-panel">
      ...
      <div className="config-actions"><button type="button"><Wrench size={16} />Editar configuração</button><button type="button"><FileText size={16} />Ver serviços</button></div>
    </div>}
  </article>
}
