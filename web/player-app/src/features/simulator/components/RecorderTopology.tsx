...
export function RecorderTopology({ mode, status }: { mode: SimulatorMode; status: SimStatus }) {
  const recording = mode === 'capture' ? 'Capturando do gravador corrente' : status === 'running' ? 'Gravando simulação' : 'Pronto para simulação'
  return <section className="recorder-column" aria-label="Gravador e Gateway">
    <div className="recorder-card central">
      ...
      <dl><div><dt>IP</dt><dd>10.10.0.10</dd></div><div><dt>RTSP</dt><dd>8554</dd></div><div><dt>Status</dt><dd>{recording}</dd></div></dl>
      <div className="health-list"><span><i />Ingestão RTSP OK</span><span><i />Writer MXF OK</span><span><i />Indexer OK</span><span><i />File Manager OK</span></div>
    </div>
